...
def add_new_person(name, img1, img2, img3):
    ...
    if (a.shape[0] - 1 != int(nid)):
        return 'ERROR: Something goes wrong with adding this new person, current embedding shape: ' + str(
            a.shape)
...
def checkin(img):
    ...
    image_id, image_list, distances, person_id = u.return_id_imgs(
        img, model_interf, face_crop)
    person_name = u.find_string_by_person_id(person_id)
    return "Checked in: " + person_name


#interface 1
app1 = gr.Interface(fn=add_new_person,
                    inputs=[
                        "text",
                        gr.Image(type="numpy"),
                        gr.Image(type="numpy"),
...
demo = gr.TabbedInterface(
    [app1, app2], ["Register Your Attendance", "Check Your Attendance"])
